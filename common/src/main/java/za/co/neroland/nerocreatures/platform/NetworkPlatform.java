package za.co.neroland.nerocreatures.platform;
 ...
 * {@link za.co.neroland.nerocreatures.network.CreatureNetwork}; each loader module registers
 * them on its own channel and implements this send interface, resolved via
 * {@link Services#NETWORK}.
 *
 * <h2>Why NeroCreatures ships its own seam rather than reusing Neroland Core's</h2>
 *
 * <p>Core's {@code za.co.neroland.nerolandcore.platform.NetworkPlatform} is public and stable,
 * and its {@code sendToPlayer} would happily carry a NeroCreatures payload. Payload
 ...
 * protocol version, coupling NeroCreatures' wire format to Core's.
 *
 * <p>NeroCreatures therefore replicates Core's architecture 1:1 on its own channel
 * ({@code nerocreatures:main}): the same declare-once/register-per-loader split, the same
 * handler shapes, the same client/common registration split on Fabric. No behaviour is copied
